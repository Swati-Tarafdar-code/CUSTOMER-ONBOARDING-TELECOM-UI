...
const VerifyUser = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (mounted) {
          setUser(res.data);
          setReviewerComment(res.data?.reviewer_comment ?? res.data?.reviewComment ?? '');
        }
      } catch (err) {
  ...
  // Submit comment + status together. Comment must not be empty.
  const handleUpdate = async (newStatus) => {
    const label = newStatus.toUpperCase();
    if (!reviewerComment || !reviewerComment.trim()) {
      setError('Reviewer comment is required before approving or rejecting.');
      return;
    }
    ...
    try {
      // first save reviewer comment, then update status
      await updateUserStatus(userId, label, reviewerComment);

      setSuccess(`User ${label} and comment saved successfully.`);
      setUser((u) => (u ? { ...u, status: label, reviewer_comment: reviewerComment } : u));
  ...
  return (
    <div className="verify-user">
      ...
      <div className="verify-card">
            ...
            <table className="docs-table">
              ...
              <tbody>
                {Array.isArray(user.documents) && user.documents.length > 0 ? (
                    ...
                    <tr key={`${d.document_id || d.documentid || idx}`}>
                      ...
                      <td>
                        {d.file_path || d.fileUrl ? (
                          <a href={d.file_path || d.fileUrl} target="_blank" rel="noreferrer" className="view-link">View</a>
                        ) : '-'}
                      </td>
                    </tr>
                  ))
                  ...
                  <tr>
                    <td colSpan="7" className="no-docs">No documents submitted.</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        {/* Reviewer Comment (no separate submit) */}
        <div className="verify-comment">
          <h3>Reviewer Comment</h3>
          <div className="comment-group">
            <textarea
              className="comment-input"
              placeholder="Enter your reviewer comment here… (required)"
              value={reviewerComment}
              onChange={(e) => setReviewerComment(e.target.value)}
              rows="4"
            />
            <div className="hint" style={{ color: '#6b7280', fontSize: 13 }}>Comment will be submitted together with Approve/Reject.</div>
          </div>
        </div>
